...
export async function getPDFInfo(pdfFile: File) {
  ...
  return {
    ...
    creationDate,
    modificationDate,
    pdfVersion: (metadata.info as any)?.PDFFormatVersion || '',
    fileSize: arrayBuffer.byteLength,
    metadata: metadata.info,
    isEncrypted: (metadata.info as any)?.IsEncrypted ?? false,
    permissions: {
      printing: !(metadata.info as any)?.IsEncrypted,
      copying: !(metadata.info as any)?.IsEncrypted,
      editing: !(metadata.info as any)?.IsEncrypted
    }
  };
}
